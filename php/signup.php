   ...
   if(mysqli_num_rows($emails)>0){
    ...
    if(isset($_FILES['image'])){
     ...
     if(in_array($img_ext,$extension)==true){
      ...
      if(move_uploaded_file($tmp_name,"../uploadeImages")){
        $status="Active now";
        $random_id=rand(time(),10000000);
        $sql="INSERT INTO users (unique_id,fname,lname,email,password,image,status) VALUES('$random_id','$fname','$lname','$email','$password','$new_img_name','$status')";
